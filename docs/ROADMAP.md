...
Everything a Claude Code transcript can express, from one tool: search, reading, statistics, visualization, diff attribution, export, live view. Phases are GitHub milestones (M0 to M8). They carry an order and a definition of done, not dates.
...
- Running sessions or sending them instructions. cctr shows the command and hands the session key to an action provider; executing is ccx's job. The provider interface exists from Phase 7 so the viewer can grow that button when ccx supplies the provider
...
| Conversation tree and the current leaf | `uuid` / `parentUuid`, `last-prompt.leafUuid` (after a rewind, this is what says which branch is current) | 1 |
| Subagent conversations and their attributes | `isSidechain` / `agentId`, `<session>/subagents/agent-<id>.jsonl` and its `.meta.json` (`agentType`, `description`, `model`, `spawnDepth`) | 1 |
| Text, thinking, tool calls, tool results | `message.content[]` blocks `text` / `thinking` / `tool_use` / `tool_result` | 1, 2 |
| Compaction boundaries | `user` records with `isCompactSummary: true` | 1 |
| Session summary for the resume list (older versions) | `summary` record (`summary`, `leafUuid`); absent in 2.1.26x, replaced by `ai-title`. Needs an older fixture | 1 |
| Model, effort, stop reason | `assistant.message.model`, `effort`, `stop_reason` | 3 |
| Tokens (input, output, cache read, cache create, server tool use, iterations) | `assistant.message.usage`, aggregated per `message.id` (see notes) | 3, 4 |
| Turn duration and message count | `system` record with `subtype: turn_duration` (`durationMs`, `messageCount`) | 3, 4 |
| Session totals (cost, API time, lines changed, tool time, per-model breakdown) | `cost-state` record (`totalCostUSD`, `totalAPIDuration`, `totalAPIDurationWithoutRetries`, `totalDuration`, `totalToolDuration`, `totalLinesAdded`, `totalLinesRemoved`, `modelUsage`, `hasUnknownModelCost`) | 3 |
| Hook results | Stop hooks: `system` record `subtype: stop_hook_summary` (`hookInfos`, `hookErrors`). PreToolUse / PostToolUse: `attachment.type` `hook_success` / `hook_non_blocking_error` / `hook_additional_context`. Counting both double-counts | 3 |
| Permission mode and denials | `permission-mode` record (`bypassPermissions`, ...) and `mode` record (`normal`, ...) are different things; denials are `user.toolDenialKind` | 3 |
| Where a prompt came from (typed by a person, or from another session or task) | `user.promptSource`, `user.origin` (`{"kind":"human"}`, `{"kind":"task..."}`) | 3, 7 |
| Interrupts and queued prompts | `queue-operation` record (`enqueue` / `dequeue` / `remove`), `attachment.queued_command` | 3, 4 |
| Files edited, with before and after | `tool_use` input of Edit / Write / MultiEdit and `toolUseResult` (`oldString`, `structuredPatch`). The pre-edit content comes from here | 5 |
| Files edited outside the session | `attachment.type: edited_text_file` | 5 |
| Backup references for edits | `file-history-snapshot` record. In real data `trackedFileBackups` is always empty and the content is not in the JSONL; an optional source, used only when the reference resolves | 5 |
| Pull requests | `pr-link` record (`prNumber`, `prUrl`, `prRepository`). Used before any inference from git log | 5 |
| Bash commands run | `tool_use` input of Bash | 2, 3 |
| Skills and slash commands used | `Skill` tool_use `input.skill`, `<command-name>` tag on user records, `attributionSkill` | 3 |
| Remote Control linkage | `bridge-session` record (`bridgeSessionId`, `ownerAccountUuid`, `ownerOrganizationUuid`; the last two are redaction targets), `attachment.type: remote_session_change` | 6, 7 |
| Context growth | `attachment.type: total_tokens_reminder` / `session_context` / `environment` / `model` | 3, 4 |
...
| Session primary key | `(machine, user, session_id)` | Same shape as ccx's `SessionKey`; keying on `session_id` alone means re-cutting the schema in Phase 8 |
...
| Agent API | HTTP + JSON, four routes, `127.0.0.1:7411` by default; any other bind is explicit and still token-gated | The contract is expected to move to Connect + protobuf (ccx's), so the surface is kept to four routes until then |
...
| Redaction | Export and MCP output are redacted by default; the CLI prints raw and takes `--redact`. A minimal detector (usernames in paths, uuid-shaped ids, token shapes) sits on the MCP path from Phase 0 | MCP answers land in Claude's context. The CLI is read by the person whose files these are |
...
Purpose: the core, the agent, and the skeleton of all four surfaces. `cctr sessions list` returns the same answer from every surface, and from every registered machine.
...
- Agent: `cctr agent run|start|stop|status|token`, routes `/meta` `/sessions` `/sessions/:id` `/sessions/:id/records`, bearer token, self-description on `/meta`
- Remotes: `cctr remote add <name> --url|--ssh`, `--host <name|all|list>` on every read
...
- MCP skeleton: stdio, one tool `sessions_list`, through the minimal redactor
- Web skeleton: `cctr serve`, one screen listing sessions
- TUI skeleton: `cctr tui`, one screen listing sessions, on Ink
- Distribution: `bun build --compile`, `scripts/install.sh` with checksum, GitHub Releases

Done when

- Parser tests pass on fixtures from at least three Claude Code versions
- An E2E shows the same session list from CLI, MCP, Web, TUI, and from a second machine over both HTTP and SSH
- `curl ... | sh` installs a working binary on a fresh machine

## Phase 1: reading

Purpose: one session, readable by a person. Replaces cchist's `show` / `outline` / `read`.
...
- Output matches or exceeds cchist's `show` / `outline` / `read` (differences recorded in the migration guide)
- A 50 MB session, read directly without the index, shows its first screen in the Web UI within 2 seconds

## Phase 2: search

Purpose: the whole history, in seconds.
...
Purpose: replace all of cchist's statistics, then deprecate cchist.
...
- Web: a numbers dashboard (tables; charts are Phase 4)
- MCP: `stats`

Done when

- Every cchist subcommand has a cctr counterpart and the migration guide's table is complete
...
- File contact graph: sessions x files (moved here from Phase 4; the data is produced here)
...
- MCP: every answer passes through redaction (in place since Phase 0)
...
## Phase 8: across machines

Purpose: several agents, pulled together. ccx does the pulling; cctr finishes what it needs.

- `/meta` carries machine id, hostname and schema version (in place since Phase 0)
...
- Absorption notes: which subcommand lands where in ccx; the agent gives way to `ccxd`, and its HTTP routes become `ccxd`'s Connect services
- ccx's action provider: start a session on a remote machine from the viewer and send it instructions, with the agent process managing it (the shape herdr uses for panes). The provider interface was fixed in Phase 7
...
- Subcommand names carry over. Differences (JSON by default, `sessions latest` printing JSON unless `--format id`, usage no longer double-counted) go in `docs/migration-from-cchist.md` when Phase 3 completes
