...
export function registerAgent(program: Command): void {
  ...
  agent
    ...
    .action(async (opts: { port: string; bind: string }) => {
      ...
      const app = createApp({ source: localSource(), token, version: VERSION });
  ...
  agent
    ...
    .action(async (opts: { port: string; bind: string }) => {
      ...
      const port = parsePort(opts.port);
      // an agent binds on this machine; a name or an address elsewhere is a mistake, not a request
      ...
      if (port !== 0) {
        ...
        const adopted = await adoptOrphan(opts.bind, port);
        if (adopted) return json({ ...describe(adopted), adopted: true });
      ...
      if (st) return json(describe(st));
      // between the port check and the child's bind someone else may have taken the port;
      // if that someone is one of our agents (a start racing this one), it is the answer
      if (port !== 0) {
        const adopted = await adoptOrphan(opts.bind, port);
        if (adopted) return json({ ...describe(adopted), adopted: true });
        if (await portInUse(opts.bind, port)) {
  ...
  agent
    ...
    .action(async () => {
      ...
      json({ running: true, ...describe(st), version: st.version });
    });
...
function describe(st: AgentState): { pid: number; url: string; startedAt: string } {
  return { pid: st.pid, url: `http://${probeHost(st.bind)}:${st.port}`, startedAt: st.startedAt };
}
...
export function isLocalBind(bind: string, ifaces = networkInterfaces()): boolean {
  const b = canonicalIp(bind);
  if (b === "0.0.0.0" || b === "::" || bind === "localhost" || b === "::1") return true;
  if (isIP(b) === 0) return false;
  if (b.startsWith("127.")) return true;
  for (const list of Object.values(ifaces)) {
    for (const i of list ?? []) if (canonicalIp(i.address.split("%")[0]!) === b) return true;
  }
  return false;
}

/** `0:0:0:0:0:0:0:1` and `::1` are the same address; the URL parser gives one spelling for both. */
function canonicalIp(addr: string): string {
...
export function isAgentProcess(pid: number, ps = readPsArgs): boolean {
  const args = ps(pid);
  // anchored at the start: `cctr` must be the executable itself, so neither
  // `cctr search "agent run"` nor `sh -c "cctr agent run"` reads as an agent
  return args !== null && /^(?:\S*\/)?cctr\s+agent\s+run(?:\s|$)/.test(args.trim());
}
...
async function healthOf(bind: string, port: number): Promise<{ name?: string; pid?: number; version?: string } | null> {
  try {
    const res = await fetch(`http://${probeHost(bind)}:${port}/healthz`, { signal: AbortSignal.timeout(1500) });
    return res.ok ? ((await res.json()) as { name?: string; pid?: number; version?: string }) : null;
  } catch {
    return null;
  }
}
...
/**
 * State file whose process is alive and ours, and whose port answers /healthz with that
 * very pid. Nothing secret is sent to find out, so a stranger on the port learns nothing.
 * A stale file (crash, reboot) reads as not running.
 */
async function liveState(): Promise<AgentState | null> {
  const st = loadAgentState();
  if (!st) return null;
  if (!alive(st.pid)) return null;
  const h = await healthOf(st.bind, st.port);
  return h?.name === "cctr" && h.pid === st.pid ? st : null;
}

/**
 * An agent of ours holding bind:port with no state file: /healthz names a pid that is
 * alive and ours. Its state is rewritten and returned; anything else is null.
 */
